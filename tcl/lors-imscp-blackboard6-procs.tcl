...
#	set txt [ad_html_text_convert -from "text/plain" -to "text/html" $txt]
    ...
    puts -nonewline $f_handler "<!-- Generated by LORS importing from Blackboard --> \ $txt \<!-- Finish importing -->"
    close $f_handler
...
ad_proc -public lors::imscp::bb6::looks_like_html_p {
    ...
    if { [regexp -nocase {<p>} $text] || [regexp -nocase {<br>} $text] || [regexp -nocase {</a} $text] || [regexp -nocase {</p>} $text] || [regexp -nocase {</html>} $text] } {
	return 1
    } else {
	return 0
    }
...
ad_proc -public lors::imscp::bb6::create_MD {
	...
	set md [lors::imsmd::create::md -owner $metadata \
		    -schema "IMS Content" \
		    -schemaversion "1.1.2" \
		    -lom $lom_node \
		   ]
...
ad_proc -public lors::imscp::bb6::clean_items {
	...
	lappend resources_list [list [lors::imsmd::getAtt $resource identifier] [lors::imsmd::getAtt $resource type] $resource $enabled_p]
...
#    ns_log Notice "HERE IS THE LIST OF ITEMS: ([llength $items_list]) $items_list \nResource list ([llength $resources_list]) $resources_list"
	...
	if {[lindex $resource 3] == "false"} {
		...
		"course/x-bb-coursetoc" {
		    ...
		    ns_log Notice "Item: [lindex $item 0] [lindex $item 1] ([lindex $item 2]) has [llength [[lindex $item 2] child all item]]] items"
		    if {[empty_string_p [[lindex $item 2] child all item]]} {
...
ad_proc -public lors::imscp::bb6::extract_html {
	...
	switch $res_type {
		...
		# set content folder to be the appropiate IMS webcontent
		...
		if { $resourcext(ISFOLDER) == "true" } {
			...
			if {$resourcext(TEXT) != "{}"} {
			    ...
			    if {![lors::imscp::bb6::looks_like_html_p -text [lindex $resourcext(TEXT) 0]]} {
				set content [ad_html_text_convert -from "text/plain" -to "text/html" [lindex $resourcext(TEXT) 0]]
			    } else {
				set content [lindex $resourcext(TEXT) 0]
			    }
