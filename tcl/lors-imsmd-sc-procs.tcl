...
namespace eval lors::imsmd::sc {
	...
	db_foreach title {select title_l, title_s from ims_md_general_title where ims_md_id = :ims_md_id} {
	...
	db_foreach description {select descrip_l, descrip_s from ims_md_general_desc where ims_md_id = :ims_md_id} {
	...
	set keywords ""
	db_foreach keyword {select keyword_l, keyword_s from ims_md_general_key where ims_md_id = :ims_md_id} {
	...
	set data_list [list object_id $object_id title $titles content [concat $titles $descriptions $keywords] mime $mime keywords $keywords storage_type $storage_type]
