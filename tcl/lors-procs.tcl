...
ad_proc -public lors::object_url {
    ...
    switch $url {
        admin {
		    ...
		    set assessment_id ""
		    set sort_order ""
		    db_0or1row get_assessment_id "select ci.item_id as assessment_id, sort_order from cr_items ci, cr_revisions cr, as_assessment_section_map m where cr.item_id=:object_id and cr.revision_id=m.section_id and ci.latest_revision=assessment_id"

                    return [export_vars \
                                -base questions \
                                {man_id assessment_id}][ad_decode {$sort_order eq ""} 0 "\#${sort_order}" ""]
                }
                "::xowiki::Page" {
		    ...
		    if {[catch {set url [[$page set package_id] make_link -privilege public -link $url $page edit ""]} errmsg]} {
		...
		default {
		    ...
		    return [export_vars -base "item-add-edit" {man_id item_id {return_url [ad_return_url]}}]
		}
            }
